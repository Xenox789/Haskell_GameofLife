...
livingNeighbors [] (a,b) = 0
livingNeighbors (x:xs) (a,b) 
        |elem x (neighbors (a,b)) = 1 + livingNeighbors xs (a,b)
        |otherwise = livingNeighbors xs (a,b)

staysAlive :: Generation -> Coordinate -> Bool
staysAlive xs (a,b)
        | livingNeighbors xs (a,b) == 2 || livingNeighbors xs (a,b) == 3 = True
        | otherwise = False



stepLivingCells :: Generation -> Generation
stepLivingCells xs = filter (\x -> livingNeighbors xs x ==2 || livingNeighbors xs x ==3) xs
...
deadNeighbors xs = unique $ sort (filter (`notElem` xs) $ concat (map (\ x-> neighbors x) xs))


stepDeadCells :: Generation -> Generation
stepDeadCells xs = unique $ filter (\ x -> livingNeighbors xs x ==3 && x `notElem` xs) $ concat $ map (neighbors) xs


stepCells :: Generation -> Generation
stepCells xs = sort (stepLivingCells xs ++ stepDeadCells xs)
...
play xs n 
        | n < 0 = Nothing
        | n == 0 = Just xs
        | otherwise = play (stepCells xs) (n-1) 
-}
